...
const aliases: Record<string, string[]> = {
	...
	"Additional transportation-safety funding": [
	...
	"Grandview Highway and Southeast Marine social housing": [
		"2518 2540 grandview",
		"1925 southeast marine",
	],
	...
	"520-590 West 29th and 4510-4550 Ash rezoning": [
		"520 590 west 29th",
		"4510 4550 ash",
	],
...
async function main() {
	...
	const scorecard = parse(scorecardSource, {
		columns: true,
		skip_empty_lines: true,
	}) as Row[]
	const metadataRows = parse(metadataSource, {
		columns: true,
		delimiter: ";",
		skip_empty_lines: true,
	}) as Row[]
	...
	const output = scorecard.map((row) => {
		...
		const close = scored.filter(
			({ item, score }) =>
				score >= (scored[0]?.score ?? 0) - 15 &&
				daysBetween(scorecardDate, item.date) <= 1460,
		)
		const selected = close.slice(0, 20)
		const voteIds = [
			...new Set(selected.map(({ item }) => item["Vote Number"])),
		]
	...
	console.log(
		`Matched ${output.length - unmatched.length} of ${output.length} scorecard votes`,
	)
	console.log(
		`Unmatched: ${unmatched.length}; multiple candidate vote IDs: ${ambiguous.length}`,
	)
