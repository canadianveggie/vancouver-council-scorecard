...
const sources = args.filter(
	(arg, index) =>
		arg !== "--out" && (outputFlag < 0 || index !== outputFlag + 1),
)

if (sources.length === 0) {
	console.error(
		"Usage: pnpm agenda:speakers <agenda-url-or-file> [more sources] [--out output.csv]",
	)
	process.exit(1)
}
...
function meetingDate(html: string, source: string) {
	...
	const match = text.match(
		/([A-Z][a-z]+ \d{1,2})(?:,\s*\d{1,2})?(?:\s+and\s+\d{1,2})?,\s*(\d{4})/,
	)
...
async function loadSource(source: string) {
	if (/^https?:\/\//i.test(source)) {
		const response = await fetch(source, {
			headers: { "User-Agent": "VancouverCouncilScorecard/1.0" },
		})
		if (!response.ok)
			throw new Error(
...
const output = `${[
	...
	...rows.map((row) =>
		[row.date, row.item, row.speakers, row.source].map(csv).join(","),
	),
].join("\n")}\n`
