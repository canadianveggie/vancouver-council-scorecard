...
function validateMetadata(parties: Party[], councillors: Councillor[]) {
	...
	for (const councillor of councillors) {
		if (!councillor.id || !councillor.name)
			error(`Councillor is missing an id or name`)
		if (councillorIds.has(councillor.id))
			error(`Duplicate councillor id: ${councillor.id}`)
...
async function build() {
	...
	const sourceCouncillorNames = sourceHeaders.filter(
		(header) => !metadataHeaders.includes(header),
	)
	const councillorsByName = new Map(
	...
	for (const name of sourceCouncillorNames) {
		if (!councillorsByName.has(name))
			error(`CSV councillor column has no metadata record: ${name}`)
	}
	for (const councillor of councillors) {
	...
	const votes: Vote[] = sourceRows.map((row, index) => {
		...
		if (usedVoteIds.has(id))
			error(`Row ${rowNumber}: duplicate generated vote id ${id}`)
		usedVoteIds.add(id)
		...
		for (const name of sourceCouncillorNames) {
			...
			councillorVotes[councillor.id] = parseRecordedVote(
				row[name]?.trim() ?? "",
				name,
				rowNumber,
			)
		...
		return {
			...
			desiredOutcome: parseOutcome(
				required(row, "Desired", rowNumber),
				rowNumber,
			),
