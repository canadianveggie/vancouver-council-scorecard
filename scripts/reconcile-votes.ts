...
async function main() {
  ...
  const minutesMismatches: Array<{ vote: string; existing: string; expected: string }> = []
  ...
  for (const vote of votes) {
    ...
    const expectedLinks = [...new Set(
      matchedCouncilVotes
        .map((row) => expectedMinutesLink(row['Meeting Type']?.trim() ?? '', row['Vote Date']?.trim() ?? ''))
        .filter(Boolean),
    )]
    const existingMinutesLink = vote['Meeting Minutes'] ?? ''
    const minutesLinkStatus = expectedLinks.length === 0
  ...
  for (const mismatch of minutesMismatches) {
    console.log(`- ${mismatch.vote}: ${mismatch.existing} != ${mismatch.expected}`)
  }
