...
async function summarize() {
	...
	const recordedVoteValues = new Set([
		"Supported",
		"Opposed",
		"Abstained",
		"Absent",
	...
	const knownVotes = rows.flatMap((row) =>
		councillors.map((name) => row[name]?.trim()).filter(Boolean),
	)
	...
	printTable(
		"Category totals",
		categoryTotals.map(([label, count]) => [label, count]),
	)
	...
	printTable("Coverage checks", [
		...
		[
			"Unknown councillor cells",
			rows.length * councillors.length - knownVotes.length,
		],
	])
	...
	const councillorRows = councillors.map((name) => {
		...
		const recorded = values.filter((value) =>
			recordedVoteValues.has(value),
		).length
		return [
			name,
			`${recorded} recorded; ${proposed} proposed; ${supported} supported; ${opposed} opposed; ${amended} amended; ${absent} absent`,
		] as [string, string]
	})
